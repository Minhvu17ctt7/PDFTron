...
const PrepareDocument = () => {
    ...
    const assignees = [
        { email: 'rrfpb.tuan1@inbox.testmail.app', name: 'Tuan 1' },
        { email: 'rrfpb.tuan2@inbox.testmail.app', name: 'Tuan 2' },
    ];
            ...
            annotationsList.map(async (annot, index) => {
                ...
                if (typeof annot.custom !== 'undefined') {
                    ...
                    if (annot.custom.type === 'TEXT') {
                        ...
                        inputAnnot = new Annotations.TextWidgetAnnotation(
                            field
                        );
                        ...
                        inputAnnot = new Annotations.SignatureWidgetAnnotation(
                            field,
                            {
                                appearance: '_DEFAULT',
                                appearances: {
                        ...
                        inputAnnot = new Annotations.DatePickerWidgetAnnotation(
                            field
                        );
                    } else {
                ...
                Annotations.WidgetAnnotation.getCustomStyles = function (
                    widget
                ) {
                    if (
    ...
    const uploadForSigning = async () => {
        ...
        const receivers = assignees.map((assignee) => ({
            name: assignee.name,
        ...
        const json = {
            ...
            // files: [
            //   {
            //     name: filePicker.current.files[0].name + '.pdf',
            //     data: Array.from(arr),
            //   }
            // ],
            mail_title: "hop dong thue nha",
            mail_message: "vui long doc dieu khoan va ky hop dong"
        }

      documentApi.postSigning(json, file);
